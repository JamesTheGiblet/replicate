...
class RustBenchmark:
    """Benchmark Rust Replicant using cargo bench or custom runs"""
    
    def __init__(self):
        self.results = {}
    
    def run_benchmark(self, n_agents: int, ticks: int, runs: int = 3) -> dict:
        """Run Rust benchmark via cargo run --release"""
        ...
        for run in range(runs):
            ...
            try:
                ...
                print(f"  ⚠️  Rust benchmark error: {e}")
                times.append(float('inf'))
        
        return {
            "time_avg": statistics.mean([t for t in times if t != float('inf')]) if times else 0,
    ...
    for n_agents in BENCHMARK_CONFIG["agents"]:
        for ticks in BENCHMARK_CONFIG["ticks"]:
            ...
            try:
                ...
                # Get stats from result or estimate
