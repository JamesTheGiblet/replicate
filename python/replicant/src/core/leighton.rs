...
impl LeightonEngine {
    ...
    pub const K_FORAGE: f32 = 0.02;       // ~35 ticks
    pub const K_FALSE_CLAIM: f32 = 0.005; // ~139 ticks
    pub const K_ATTACK: f32 = 0.001;      // ~693 ticks
    ...
    pub const RECIDIVISM_STEP: f32 = 1.0;   // each prior offence adds 100%
    ...
    fn apply_with_recidivism(&mut self, agent_id: &str, tick: u32, delta_base: f32, k: f32, reason: &str) {
        ...
        let multiplier = multiplier.min(5.0); // cap at 5x
        let delta = delta_base * multiplier;
